              ...
              <div style="font-family: 'Noto Sans KR', sans-serif;">
                ...
                <div class="d-flex mt-3" style="space-between">
                  ...
                  <b-form-checkbox v-model="newRoom.roomVideo">
                    <small><i class="bi bi-camera-video-off-fill"></i>&nbsp;&nbsp;캠화면 끄기</small>
                  </b-form-checkbox>
                </div>

                <b-button v-b-toggle.collapse class="my-1 mt-3">방 세부 설정</b-button>
                
                <b-collapse id="collapse" class="mt-2">
                  <b-card>
                    <label for="range-1">방 최대 인원 : {{ newRoom.roomCapacity }} 명</label>
                    <b-form-input id="range-1" v-model="newRoom.roomCapacity" type="range" min="1" max="15"></b-form-input>

                    <div class="d-flex">
                      <b-form-checkbox v-model="newRoom.useAlarm">
                        <small><i class="bi bi-alarm-fill"></i>&nbsp;공부 알람 사용하기</small>
                      </b-form-checkbox>
                      <Popper arrow>
                        <i class="bi bi-question-circle ms-2"
                        style="vertical-align: middle;"></i>
                    ...
                    <div v-if="newRoom.useAlarm">
                      <label class="mt-2" for="range-1">공부 시간 : {{ newRoom.roomStudyTime }} 분</label>
                      <b-form-input id="range-1" v-model="newRoom.roomStudyTime" type="range" min="10" max="300" step="10"></b-form-input>

                      <label class="mt-2" for="range-1">쉬는 시간 : {{ newRoom.roomRestTime }} 분</label>
                      <b-form-input id="range-1" v-model="newRoom.roomRestTime" type="range" min="5" max="60" step="5"></b-form-input>
                    </div>

                    <label class="mt-3" for="formFile">방 썸네일 사진</label>
                    <input class="form-control" type="file" id="roomThumbnail" accept="image/png, image/jpeg">
                  </b-card>
                </b-collapse>
                </b-card>
                <div class="d-flex mt-2">
                  <b-button variant="success" :disabled="!isAllValid()" @click="makeRoom">완료</b-button>
...
<script>
import { ref, computed, nextTick, onBeforeMount, onMounted, reactive } from 'vue';
...
export default {
  ...
  setup() {
    ...
    const validation = reactive({
      roomName: computed(() => {
        return newRoom.roomName == null? null : new RegExp(/^.{1,20}$/).test(newRoom.roomName);
      }),
      roomPw: computed(() => {
        return newRoom.roomHasPw ? (newRoom.roomPw == null ? null : new RegExp(/^[a-zA-Z0-9]{4,10}$/).test(newRoom.roomPw)) : null
      }),
